...
- アクション意図ゲート (正規表現、forget/sensor と並置):
  `(バックアップ|TM).*(回|取っ|開始|走|実行)|(バックアップ|TM)(を|も)?(して|しといて|しておいて|お願い)|((スタジオ|studio|ミニ|mini).*(投げ|回し|任せ|やらせ|やって))`
  (2026-07-15 統合レビューで限定強化: bare「して」だと「状態を確認して」= sensor 意図まで提案化するため)
...
- 確認語が来たが実行待ちが無い場合 (破棄/期限切れ/実行済み直後の墓標あり session、または履歴ゼロの初手) はサーバ定型「実行待ちの操作はありません」で完結しモデルを呼ばない
  (2026-07-15 実LLMスモーク: モデルが実行を演技する2経路を実測したため)
